"""
Attributes : 
    tok(int) : 초기값 0, 마지막 객체 인식 한 시점의 시간(초)를 저장
    client(object) : mqtt 서버를 연결한 객체 
    devi(list) : LoadStreams에서 받아온 카메라의 이름 리스트 
    f(object) : log 파일
    message(list) : mqtt 서버가 연결이 안되었을 때 메세지를 담아 놓는 리스트 
    save_name(str) : 인식한 사진을 저장할 파일명. 현재시간_사람수 형식
    img_s(ndarray) : 원본 이미지  
Args :
    ip : mqtt 서버의 ip address
    topic : 메세지 전송할 토픽명
    img_path : 객체 인식이 되었을 경우 저장할 이미지(원본:img_s)의 경로
    log : 로그 파일의 경로
Note :
    img_path 폴더가 존재하지 않는 경우 폴더 생성
    log는 경로만 지정하고 파일명은 log.txt로 고정
    view-img(True) 일때 esc입력시 프로그램 종료 
"""
...
def run(
    ...
    if not os.path.isdir(opt.img_path) : os.mkdir(opt.img_path) # 폴더가 없는 경우, 폴더 생성
    # Directories
    ...
    for path, im, im0s, vid_cap, s, info, error in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det) and view_img: # view-img 옵션이 True이고 감지가 되었을 때 감지된 객체의 정보를 이미지에 그림.
            ...
            # 객체 인식 시 len(det) = 사람 수
            if len(det) > 0 :
                tt = datetime.datetime.now()
                tick = int(time.time())  
                if tick > tok :
                    if client.is_connected() : # mqtt 연결이 될 때
                        if len(message) > 0 : # mqtt 연결이 안되어 메세지를 못 보낸것이 있을 경우
                            for i, s in enumerate(message): client.publish(topic, s)
                            message.clear()
                        else : client.publish(topic,f'{tt},{len(det)},{devi}') # 현재시간, 사람수, 카메라 정보를 전송
                    else:
                        f = open(log_path, 'a')
                    ...
                    saves = os.path.join(img_path,save_name)
                    cv2.imwrite(saves + '.jpg',img_s) 
                    print(f'{tt},{len(det)},{devi}')
                    tok = tick 
            ...
            if view_img :
                ...
                im0 = cv2.resize(im0, dsize=(0,0), fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA) # 영역 보간법으로 사이즈 축소(1/2)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='people.pt', help='model path or triton URL')
