...
class DetectionModel(paddle.nn.Layer):
    def __init__(self):
        ...
        self.conv2d0 = paddle.nn.Conv2D(out_channels=16, kernel_size=(6, 6), stride=2, padding=2, in_channels=3)
        self.silu0 = paddle.nn.Silu()
        self.conv2d1 = paddle.nn.Conv2D(out_channels=32, kernel_size=(3, 3), stride=2, padding=1, in_channels=16)
        self.silu1 = paddle.nn.Silu()
        self.conv2d2 = paddle.nn.Conv2D(out_channels=16, kernel_size=(1, 1), in_channels=32)
        self.silu2 = paddle.nn.Silu()
        self.conv2d3 = paddle.nn.Conv2D(out_channels=16, kernel_size=(1, 1), in_channels=16)
        self.silu3 = paddle.nn.Silu()
        self.conv2d4 = paddle.nn.Conv2D(out_channels=16, kernel_size=(3, 3), padding=1, in_channels=16)
        self.silu4 = paddle.nn.Silu()
        self.conv2d5 = paddle.nn.Conv2D(out_channels=16, kernel_size=(1, 1), in_channels=32)
        self.silu5 = paddle.nn.Silu()
        self.conv2d6 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=32)
        self.silu6 = paddle.nn.Silu()
        self.conv2d7 = paddle.nn.Conv2D(out_channels=64, kernel_size=(3, 3), stride=2, padding=1, in_channels=32)
        self.silu7 = paddle.nn.Silu()
        self.conv2d8 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=64)
        self.silu8 = paddle.nn.Silu()
        self.conv2d9 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=32)
        self.silu9 = paddle.nn.Silu()
        self.conv2d10 = paddle.nn.Conv2D(out_channels=32, kernel_size=(3, 3), padding=1, in_channels=32)
        self.silu10 = paddle.nn.Silu()
        self.conv2d11 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=32)
        self.silu11 = paddle.nn.Silu()
        self.conv2d12 = paddle.nn.Conv2D(out_channels=32, kernel_size=(3, 3), padding=1, in_channels=32)
        ...
        self.conv2d15 = paddle.nn.Conv2D(out_channels=128, kernel_size=(3, 3), stride=2, padding=1, in_channels=64)
        self.silu15 = paddle.nn.Silu()
        self.conv2d16 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=128)
        self.silu16 = paddle.nn.Silu()
        self.conv2d17 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        self.silu17 = paddle.nn.Silu()
        self.conv2d18 = paddle.nn.Conv2D(out_channels=64, kernel_size=(3, 3), padding=1, in_channels=64)
        ...
        self.conv2d20 = paddle.nn.Conv2D(out_channels=64, kernel_size=(3, 3), padding=1, in_channels=64)
        self.silu20 = paddle.nn.Silu()
        self.conv2d21 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        self.silu21 = paddle.nn.Silu()
        self.conv2d22 = paddle.nn.Conv2D(out_channels=64, kernel_size=(3, 3), padding=1, in_channels=64)
        ...
        self.conv2d24 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=128)
        self.silu24 = paddle.nn.Silu()
        self.conv2d25 = paddle.nn.Conv2D(out_channels=256, kernel_size=(3, 3), stride=2, padding=1, in_channels=128)
        ...
        self.conv2d27 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=128)
        self.silu27 = paddle.nn.Silu()
        self.conv2d28 = paddle.nn.Conv2D(out_channels=128, kernel_size=(3, 3), padding=1, in_channels=128)
        self.silu28 = paddle.nn.Silu()
        self.conv2d29 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=256)
        self.silu29 = paddle.nn.Silu()
        self.conv2d30 = paddle.nn.Conv2D(out_channels=256, kernel_size=(1, 1), in_channels=256)
        ...
        self.conv2d33 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=256)
        self.silu33 = paddle.nn.Silu()
        self.conv2d34 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=256)
        self.silu34 = paddle.nn.Silu()
        self.conv2d35 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        self.silu35 = paddle.nn.Silu()
        self.conv2d36 = paddle.nn.Conv2D(out_channels=64, kernel_size=(3, 3), padding=1, in_channels=64)
        self.silu36 = paddle.nn.Silu()
        self.conv2d37 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=256)
        self.silu37 = paddle.nn.Silu()
        self.conv2d38 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=128)
        self.silu38 = paddle.nn.Silu()
        self.conv2d39 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=128)
        self.silu39 = paddle.nn.Silu()
        self.conv2d40 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=128)
        self.silu40 = paddle.nn.Silu()
        self.conv2d41 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=32)
        self.silu41 = paddle.nn.Silu()
        self.conv2d42 = paddle.nn.Conv2D(out_channels=32, kernel_size=(3, 3), padding=1, in_channels=32)
        self.silu42 = paddle.nn.Silu()
        self.conv2d43 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=128)
        self.silu43 = paddle.nn.Silu()
        self.conv2d44 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        self.silu44 = paddle.nn.Silu()
        self.conv2d45 = paddle.nn.Conv2D(out_channels=64, kernel_size=(3, 3), stride=2, padding=1, in_channels=64)
        self.silu45 = paddle.nn.Silu()
        self.conv2d46 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=128)
        ...
        self.conv2d48 = paddle.nn.Conv2D(out_channels=64, kernel_size=(3, 3), padding=1, in_channels=64)
        self.silu48 = paddle.nn.Silu()
        self.conv2d49 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=128)
        self.silu49 = paddle.nn.Silu()
        self.conv2d50 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=128)
        self.silu50 = paddle.nn.Silu()
        self.conv2d51 = paddle.nn.Conv2D(out_channels=128, kernel_size=(3, 3), stride=2, padding=1, in_channels=128)
        self.silu51 = paddle.nn.Silu()
        self.conv2d52 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=256)
        self.silu52 = paddle.nn.Silu()
        self.conv2d53 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=128)
        self.silu53 = paddle.nn.Silu()
        self.conv2d54 = paddle.nn.Conv2D(out_channels=128, kernel_size=(3, 3), padding=1, in_channels=128)
        self.silu54 = paddle.nn.Silu()
        self.conv2d55 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=256)
        ...
        self.conv2d59 = paddle.nn.Conv2D(out_channels=255, kernel_size=(1, 1), in_channels=256)
